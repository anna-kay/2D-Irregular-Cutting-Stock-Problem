...
class FigureObjects:
    ...
    def update(self, degl):
        ...
        zz = [90*(round(degl.GlobalBestPosition[(3*i)+2])%2) for i in range(len(orderItems))]
...
#-----------------------------------------------------------------------------#
    ...
    for order_num in range(0, len(myOrders)):
        ...
        # (e.g., 100 or 200).
        degl = DEGL(ObjectiveFcn, nVars,  LowerBounds=LowerBounds, UpperBounds=UpperBounds, PopulationSize=PopulationSize*(2**order_num),
        ...
        best_thetas = [90*(round(degl.GlobalBestPosition[3*i+2])%2) for i in range(len(myOrder))]
