...
class DEGL:
    ...
    def __init__(self, ObjectiveFcn, nVars, LowerBounds=None, UpperBounds=None, PopulationSize=None, CR=0.9  # crossover probability [0.4-0.9] # 0.6 -> good resultls as well
                 , aScaleFactor=0.8  # a good inital choice is 0.5 # 2.05 #
                 , bScaleFactor=0.8  # 2.05 # [0.4 - 0.95]
                 , minWeight=0.4  # 0.4 #
                 , maxWeight=0.8  # 0.9 #
                 , NeighborhoodFraction=0.1, FunctionTolerance=1.0e-6, MaxIterations=None, MaxStallIterations=20, OutputFcn=None, UseParallel=False
                 ):
        ...
        assert np.isscalar(
            aScaleFactor), "The aScaleFactor option must be a scalar number."
        self.aScaleFactor = aScaleFactor
        assert np.isscalar(
            bScaleFactor), "The bScaleFactor option must be a scalar number."
        self.bScaleFactor = bScaleFactor

        assert np.isscalar(
            minWeight), "The minWeight option must be a scalar number."
        self.minWeight = minWeight
        assert np.isscalar(
            maxWeight), "The maxWeight option must be a scalar number."
        ...
        if self.UseParallel and not HaveJoblib:
            warnings.warn(
                """If UseParallel is set to True, it requires the joblib package that could not be imported; population objective values will be computed in serial mode instead.""")
        ...
        self.Population = lbMatrix + \
            np.random.rand(nAgents, nVars) * bRangeMatrix
#
...
#        f.close()
        ...
        # Neighborhood radius
        self.NeighborhoodRadius = max(
            1, int(np.floor(nAgents * self.NeighborhoodFraction)))
        ...
        self.LocalMutation = lbMatrix + \
            np.zeros((nAgents, nVars)) * bRangeMatrix
        self.GlobalMutation = lbMatrix + \
            np.zeros((nAgents, nVars)) * bRangeMatrix
        self.CombinedMutation = lbMatrix + \
            np.zeros((nAgents, nVars)) * bRangeMatrix
    ...
    def __evaluatePopulation(self):
        ...
        if self.UseParallel:
            ...
            self.CurrentPopulationFitness[:] = [self.ObjectiveFcn(
                self.Population[i, :]) for i in range(nAgents)]
    ...
    def optimize(self):
        ...
        while not doStop:
            ...
            W = minWeight + (maxWeight - minWeight) * \
                (self.Iteration - 1)/(self.MaxIterations - 1)
            ...
            for i in range(nAgents):
                ...
                neighborsIndices = list(
                    range(i-NeighborhoodRadius, i+NeighborhoodRadius+1))

                neighborsIndices = [index %
                                    nAgents for index in neighborsIndices]
...
#                f.close()
                ...
                # TODO: check numbers!!!
                indicesToChooseFrom = list(
                    filter(lambda x: x != NeighborhoodRadius, indicesToChooseFrom))  # remove i
                p = np.random.choice(indicesToChooseFrom, 1)
                indicesToChooseFrom = list(
                    filter(lambda x: x != p, indicesToChooseFrom))
                q = np.random.choice(indicesToChooseFrom, 1)

                # 4. compute the mutation
                self.LocalMutation[i, :] = self.Population[i, :] + a*(
                    bestNeighbor-self.Population[i, :]) + b*(neighbors[p, :]-neighbors[q, :])

                # COMPUTE GLOBAL MUTATION
                # 1. select two random indices from the population
                indicesToChooseFrom = np.arange(nAgents)
                indicesToChooseFrom = list(
                    filter(lambda x: x != i, indicesToChooseFrom))
                r1 = np.random.choice(nAgents, 1)
                indicesToChooseFrom = list(
                    filter(lambda x: x != r1, indicesToChooseFrom))
                r2 = np.random.choice(nAgents, 1)

                # 2. compute the mutation
                self.GlobalMutation[i, :] = self.Population[i, :] + a*(
                    self.GlobalBestPosition-self.Population[i, :]) + b*(self.Population[r1, :]-self.Population[r2, :])

                # COMPUTE COMBINED MUTATION
                self.CombinedMutation[i, :] = W*self.GlobalMutation[i,
                                                                    :] + (1-W)*self.LocalMutation[i, :]
...
#                print("This is the ", i, " agent", self.Population[i])
            ...
            self.PreviousBestPosition[agentsProgressed,
                                      :] = self.Population[agentsProgressed, :]
