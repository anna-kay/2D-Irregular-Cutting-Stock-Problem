...
def ObjectiveFcn(particle):
    ...
    orderItemsWithBuffer = [item.buffer(
        0.1, join_style=joinStyle, cap_style=capStyle) for item in orderItems]
...
class FigureObjects:
    ...
    def update(self, pso):
        ...
        zz = [90*(round(pso.GlobalBestPosition[(3*i)+2]) % 2)
              for i in range(len(orderItems))]
        ...
        self.axBestFit.title.set_text(
            'Best-so-far global best fitness: {:g}'.format(pso.GlobalBestFitness))
...
#-----------------------------------------------------------------------------#
    ...
    for order_num in range(0, len(myOrders)):
        ...
        # (e.g., 100 or 200).
        pso = DynNeighborPSO(ObjectiveFcn, 
                             nVars,  
                             LowerBounds=LowerBounds, 
                             UpperBounds=UpperBounds, 
                             SwarmSize=SwarmSize*(order_num+1),
                             OutputFcn=outFun, 
                             UseParallel=True, 
        ...
        best_thetas = [90*(round(pso.GlobalBestPosition[3*i+2]) % 2)
